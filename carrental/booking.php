            ...
            try {
                ...
                if ($car) {
                    ...
                    <div class="row">
                        ...
                        <!-- Booking Details -->
                        <div class="col-md-6">
                            <div class="card shadow p-4 mb-5"> <!-- Added mb-5 to add space below the card -->
                                <h4 class="text-center">Booking Details</h4>
                                <p><strong>Car Name:</strong> <?= $car_name ?></p>
                                <p><strong>Price Per Day:</strong> $ <?= number_format($price_per_day, 2) ?></p>
                        ...
                        <div class="col-md-6">
                            <div class="card shadow p-4">
                                ...
                                <p><strong>Tax (10%):</strong> $ <?= number_format($tax_amount, 2) ?></p>
                                ...
                                <a href="checkout.php?car_id=<?= $car_id ?>&start_date=<?= $start_date ?>&end_date=<?= $end_date ?>&booking_ref=<?= $booking_ref ?>&total_amount=<?= $total_amount ?>"
                                    class="btn btn-success w-100">Proceed to Pay</a>
                            </div>
                        </div>
                    </div>
                    ...
                    echo "<p class='alert alert-danger'>Car details not found.</p>";
                }
            } catch (PDOException $e) {
                echo "<p class='alert alert-danger'>Error: {$e->getMessage()}</p>";
            }
        } else {
            echo "<p class='alert alert-danger'>Invalid request. Missing parameters.</p>";
        }
        ?>
    </div>
</main>
